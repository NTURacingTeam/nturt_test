...
class ProgressBar : public rclcpp::Node {
 public:
  ...
  ProgressBar(rclcpp::NodeOptions _options)
      : Node("fake_socket_can_bridge_node", _options),
        progress_bar_timer_(this->create_wall_timer(
 ...
 private:
  ...
  void progress_bar_callback() {
    int green_bar_length =
        static_cast<int>(bar_length * partial_progress_ / 100);
    int blue_bar_length = static_cast<int>(
        bar_length * (total_progress_ - partial_progress_) / 100);
    int empty_bar_length = bar_length - green_bar_length - blue_bar_length;
    std::cout << "progress: [" << green << std::string(green_bar_length, '|')
              << blue << std::string(blue_bar_length, '|') << reset
              << std::string(empty_bar_length, ' ') << ']' << total_progress_
              << "%\r";
    std::cout.flush();
...
int main(int argc, char **argv) {
  ...
  rclcpp::Node::SharedPtr progress_bar_node =
      std::make_shared<ProgressBar>(options);
