...
/// @brief Union for type plunning between test frame information and raw frame
/// data.
...
class RpiCanLatencyTest : public rclcpp::Node {
 ...
 private:
  ...
  /// @brief ROS2 timer for starting can latency test, wait a second for
  /// everything to initialize before starting the test.
  rclcpp::TimerBase::SharedPtr starting_timer_;

  /// @brief ROS2 timer for periodically showing the test progress, also used
  /// for showing statistics of the echo server.
  ...
  /// @brief Time stamp when this program starts, used for increasing the
  /// resolution of measured lateency.
  double program_start_time_;

  /// @brief Total test message to sent, calculated by
  /// $text{test_length}/text{test_period}$.
  int total_count_;
  ...
  /// @brief Counter counting how many can singals were received, used for echo
  /// server.
  int received_echo_count_ = 0;

  /// @brief Counter counting how many can singals were received last time
  /// progress_callback is called, used for echo server.
  int last_received_echo_count_ = 0;

  /// @brief Buffer for storing the data before flusing to csv file at the end
  /// of the test.
  ...
  /// @brief Timed callback function for starting can latency test, wait a
  /// second for everything to initialize before starting the test.
  void starting_callback();

  /// @brief Timed callback function for showing test progress, also used for
  /// showing statistics of the echo server.
