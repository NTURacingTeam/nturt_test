...
RpiCanLatencyTest::RpiCanLatencyTest(rclcpp::NodeOptions _options)
    : Node("nturt_rpi_can_latency_test_node", _options),
      ...
      can_latency_test_timer_(this->create_wall_timer(
          std::chrono::duration<double>(
              this->declare_parameter("test_period", 0.01)),
          std::bind(&RpiCanLatencyTest::can_latency_test_callback, this))),
      starting_timer_(this->create_wall_timer(
  ...
  if (!is_echo_server_) {
    ...
    test_period_ =
        this->get_parameter("test_period").get_parameter_value().get<double>();
    ...
    strftime(default_file_name, sizeof(default_file_name),
             "%Y-%m-%d-%H-%M-%S.csv", std::localtime(&now));
    std::string logging_file_name =
        this->declare_parameter("logging_file_name", default_file_name);

    RCLCPP_INFO(this->get_logger(),
                "The test will be running for %fs at %fs per test message.",
                test_length_, test_period_);

    // register onShutdown to ros
    rclcpp::on_shutdown(std::bind(&RpiCanLatencyTest::onShutdown, this));

    // open csv_file
    std::string file = ament_index_cpp::get_package_share_directory(
                           "nturt_rpi_can_latency_test") +
                       "/" + logging_file_name;
    ...
    // start timer
    RCLCPP_INFO(this->get_logger(),
                "Wait a second for everything to initialize before starting "
                "the test...");
    ...
    RCLCPP_INFO(this->get_logger(),
                "This test is configured as echo server. Ctrl + C to stop.");
  }
}
...
void RpiCanLatencyTest::onCan(const can_msgs::msg::Frame &_msg) {
  if (!is_echo_server_ && _msg.id == receive_id_) {
    ...
    float latency = static_cast<float>(now - program_start_time_) -
                    compose.information.time_stemp_;

    // log to write_buffer_
    write_buffer_index_ +=
        sprintf(write_buffer_.get() + sizeof(char) * write_buffer_index_,
                "%d,%f\n", frame_count, latency);
    received_count_++;
...
void RpiCanLatencyTest::can_latency_test_callback() {
  ...
  compose.information.time_stemp_ =
      static_cast<float>(this->now().seconds() - program_start_time_);
  ...
  if (sent_count_ == total_count_) {
    ...
    progress_timer_->cancel();
    RCLCPP_INFO(this->get_logger(),
                "Rpi can latency test complete, waiting 1 second for not yet "
                "arrived frames...");
    rclcpp::sleep_for(1s);
...
void RpiCanLatencyTest::progress_callback() {
  if (!is_echo_server_) {
    int green_bar_length =
        static_cast<int>(bar_length * received_count_ / total_count_);
    int blue_bar_length = static_cast<int>(
        bar_length * (sent_count_ - received_count_) / total_count_);
    int empty_bar_length = bar_length - green_bar_length - blue_bar_length;
    terminal_ << std::fixed << "progress: [" << green
              << std::string(green_bar_length, '|') << blue
              << std::string(blue_bar_length, '|') << reset
              << std::string(empty_bar_length, ' ') << "] sent: " << blue
              << sent_count_ << '(' << std::setprecision(1)
              << 100.0 * sent_count_ / total_count_ << "%)" << reset
              << " received: " << green << received_count_ << '('
              << std::setprecision(1) << 100.0 * received_count_ / total_count_
              << "%)" << reset << " total: " << total_count_
              << " receive rate: " << std::setprecision(5)
              << 100.0 * received_count_ / sent_count_ << "%\r";
    terminal_.flush();
  } else {
    int count = received_echo_count_ - last_received_echo_count_;
    last_received_echo_count_ = received_echo_count_;

    terminal_ << std::fixed << "Received: " << std::string(5 - count, ' ')
              << count << " messages during last second.\r";
    terminal_.flush();
  }
}
